import type { CooldownGroup, Skill } from '../../../model/types';

export const SGE_COOLDOWN_GROUPS: CooldownGroup[] = [
  {
    id: 'sge-grp-addersgall',
    stack: 3,
    recovery: { cooldownSec: 20 },
    resourceDisplay: { label: '蛇胆' },
  },
];

export const SGE_SKILLS: Skill[] = [
  {
    id: 'sge-rhizomata',
    name: '根素',
    name_en: 'Rhizomata',
    name_jp: 'リゾーマタ',
    name_fr: 'Rizomata',
    name_de: 'Rizomata',
    cooldownSec: 90,
    durationSec: 0,
    job: 'SGE',
    actionId: 24309,
    cooldownGroupRecoveries: [{ groupId: 'sge-grp-addersgall', amount: 1 }],
  },
  {
    id: 'sge-physis-ii',
    name: '自生II',
    name_en: 'Physis II',
    name_jp: 'ピュシスII',
    name_fr: 'Physis II',
    name_de: 'Physis II',
    cooldownSec: 60,
    durationSec: 15,
    mitigation: [{ kind: 'hot', durationSec: 15, potency: 130, targeting: { kind: 'party' } }],
    job: 'SGE',
    actionId: 24302,
  },
  {
    id: 'sge-druochole',
    name: '灵橡清汁',
    name_en: 'Druochole',
    name_jp: 'ドルオコレ',
    name_fr: 'Druochole',
    name_de: 'Druochole',
    cooldownSec: 1,
    durationSec: 0,
    job: 'SGE',
    actionId: 24296,
    cooldownGroup: ['sge-grp-addersgall'],
  },
  {
    id: 'sge-ixochole',
    name: '寄生清汁',
    name_en: 'Ixochole',
    name_jp: 'イックソコレ',
    name_fr: 'Ixochole',
    name_de: 'Ixochole',
    cooldownSec: 30,
    durationSec: 0,
    job: 'SGE',
    actionId: 24299,
    cooldownGroup: ['sge-grp-addersgall'],
  },
  {
    id: 'sge-kerachole',
    name: '坚角清汁',
    name_en: 'Kerachole',
    name_jp: 'ケーラコレ',
    name_fr: 'Kerachole',
    name_de: 'Kerachole',
    cooldownSec: 30,
    durationSec: 15,
    mitigation: [
      {
        kind: 'damage-down',
        target: 'player',
        pct: 10,
        durationSec: 15,
        damageType: 'all',
        targeting: { kind: 'party' },
      },
      { kind: 'hot', durationSec: 15, potency: 100, targeting: { kind: 'party' } },
    ],
    job: 'SGE',
    actionId: 24298,
    cooldownGroup: ['sge-grp-addersgall'],
  },
  {
    id: 'sge-taurochole',
    name: '白牛清汁',
    name_en: 'Taurochole',
    name_jp: 'タウロコレ',
    name_fr: 'Taurochole',
    name_de: 'Taurochole',
    cooldownSec: 45,
    durationSec: 15,
    mitigation: [
      {
        kind: 'damage-down',
        target: 'player',
        pct: 10,
        durationSec: 15,
        damageType: 'all',
        targeting: { kind: 'ally', maxTargets: 1, canTargetSelf: true },
      },
    ],
    job: 'SGE',
    actionId: 24303,
    cooldownGroup: ['sge-grp-addersgall'],
  },
  {
    id: 'sge-haima',
    name: '输血',
    name_en: 'Haima',
    name_jp: 'ハイマ',
    name_fr: 'Haima',
    name_de: 'Haima',
    cooldownSec: 120,
    durationSec: 15,
    mitigation: [
      {
        kind: 'shield',
        durationSec: 15,
        potency: 300,
        targeting: { kind: 'ally', maxTargets: 1, canTargetSelf: true },
      },
    ],
    job: 'SGE',
    actionId: 24305,
  },
  {
    id: 'sge-panhaima',
    name: '泛输血',
    name_en: 'Panhaima',
    name_jp: 'パンハイマ',
    name_fr: 'Panhaima',
    name_de: 'Panhaima',
    cooldownSec: 120,
    durationSec: 15,
    mitigation: [{ kind: 'shield', durationSec: 15, potency: 200, targeting: { kind: 'party' } }],
    job: 'SGE',
    actionId: 24311,
  },
  {
    id: 'sge-holos',
    name: '整体论',
    name_en: 'Holos',
    name_jp: 'ホーリズム',
    name_fr: 'Holos',
    name_de: 'Holos',
    cooldownSec: 120,
    durationSec: 20,
    mitigation: [
      {
        kind: 'damage-down',
        target: 'player',
        pct: 10,
        durationSec: 20,
        damageType: 'all',
        targeting: { kind: 'party' },
      },
      { kind: 'shield', durationSec: 30, potency: 300, targeting: { kind: 'party' } },
    ],
    job: 'SGE',
    actionId: 24310,
  },
  {
    id: 'sge-pneuma',
    name: '魂灵风息',
    name_en: 'Pneuma',
    name_jp: 'プネウマ',
    name_fr: 'Pneuma',
    name_de: 'Pneuma',
    cooldownSec: 120,
    durationSec: 0,
    job: 'SGE',
    actionId: 24318,
  },
  {
    id: 'sge-philosophia',
    name: '智慧之爱',
    name_en: 'Philosophia',
    name_jp: 'フィロソフィア',
    name_fr: 'Philosophia',
    name_de: 'Philosophia',
    cooldownSec: 180,
    durationSec: 20,
    job: 'SGE',
    actionId: 37035,
  },
  {
    id: 'sge-zoe',
    name: '活化',
    name_en: 'Zoe',
    name_jp: 'ゾーエ',
    name_fr: 'Zoe',
    name_de: 'Zoe',
    cooldownSec: 90,
    durationSec: 30,
    job: 'SGE',
    actionId: 24300,
  },
  {
    id: 'sge-soteria',
    name: '拯救',
    name_en: 'Soteria',
    name_jp: 'ソーテリア',
    name_fr: 'Soteria',
    name_de: 'Soteria',
    cooldownSec: 60,
    durationSec: 15,
    job: 'SGE',
    actionId: 24294,
  },
];
