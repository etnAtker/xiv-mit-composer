...
- **冷却校验、资源档数与覆盖提示**：按技能、共享冷却组和释放者校验冷却，时间轴中显示共享资源档数、不可用区间（黄）、技能冷却区间（灰）和冲突技能投影。
...
3.  **排轴操作**：
    ...
    - **持续结束**：部分技能可以提前结束另一个技能的持续时间。将持续结束型技能拖入同一玩家的父技能原始持续窗口时，会在父减伤条右下角生成结束标记，并把父减伤条缩短到落点时间。
    - **调整结束标记**：拖拽结束标记可以调整结束时间；右键结束标记可以编辑事件或删除结束标记。父技能编辑弹窗中的结束时间留空时，会移除结束标记并恢复完整持续时间。
    - **缩放视图**：按住 `Alt` + 滚轮缩放时间轴，也可以使用时间轴工具栏调整 `px/s`。
    - **折叠分组**：战斗信息栏提供全部展开、全部折叠和调整队伍入口。
    - **资源档数**：启用资源显示的共享冷却组会在对应玩家技能组内显示资源档数列。色带表示当前层数，列头显示资源短标签，层数变化点显示变化后的档数。
