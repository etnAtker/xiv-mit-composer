...
玩家选择窗口提供添加一名玩家和添加一个职能两种模式。用户最多选择 8 个成员，不校验职业构成，允许重复职业，并能调整已选成员顺序。添加玩家会加载该玩家的友方咏唱并生成已有减伤轴；添加职能会生成空白减伤轴，空白成员名称与职能名一致。确认后，应用按所选真实玩家加载友方咏唱，并按当前战斗中所有可识别玩家加载受击事件和 Boss 咏唱事件。
